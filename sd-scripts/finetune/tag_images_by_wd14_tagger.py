...
def main(args):
    ...
    logger.info(f"Using local wd14 tagger model from {model_location}")

    safetensors_file = find_safetensors_file(model_location)
    ...
    if args.onnx:
        ...
        onnx_file = find_onnx_file(model_location)
        if not onnx_file:
            raise FileNotFoundError(f"ONNX file not found in {model_location}")
        model_path = os.path.join(model_location, onnx_file)
        logger.info(f"Creating ONNX InferenceSession from {model_path}")
        ...
        model_path = os.path.join(model_location, safetensors_file)
    ...
    csv_file_path = find_csv_file(model_location)
    if not csv_file_path:
        raise FileNotFoundError(f"CSV file not found in {model_location}")
    logger.info(f"Loading tags from {csv_file_path}")

    with open(os.path.join(model_location, csv_file_path), "r", encoding="utf-8") as f:
        reader = csv.reader(f)
    ...
    # --- DEBUG ---
    logger.info(f"Loaded CSV file from: {os.path.join(model_location, CSV_FILE)}")
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model_dir", type=str, required=True, help="directory containing wd14 tagger model / wd14 taggerのモデルを格納したディレクトリ")
